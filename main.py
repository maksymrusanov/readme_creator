...
def get_repo_url() -> str | None:
    url = input(
        "Enter the GitHub repository URL (or press Enter to skip): ").strip()
    if url == "exit":
...
def main(url: str | None = None):
    ...
    if not response.text:
        ...
        if url:
            ...
            raise ValueError("No URL provided, cannot generate README.md")
    path = Path(f"~/projects/readmes/{repo_name}").expanduser()
    path.mkdir(parents=True, exist_ok=True)
    with open(path / filename, "w") as f:
        f.write(response.text)
    print(f"{filename} written to {path/filename}")
